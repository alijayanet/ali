...
if ! check_node_version; then
    ...
    NODE_VERSION=$(node -v | cut -d 'v' -f 2)
    echo -e "${GREEN}============================================================================${NC}"
    echo -e "${GREEN}============== NodeJS wis di install versi ${NODE_VERSION}. ================${NC}"
...
if !  systemctl is-active --quiet mongod; then
    ...
    curl -fsSL https://www.mongodb.org/static/pgp/server-3.4.asc | apt-key add -
    apt-key list
    echo "deb [ arch=amd64,arm64 ] http://repo.mongodb.org jessie/mongodb-org/3.4 Release.gpg" | tee /etc/apt/sources.list.d/mongodb-org-4.4.list
    apt update
    ...
    echo -e "${GREEN}============================================================================${NC}"
    echo -e "${GREEN}=================== mongodb wis terinstall sedurunge. ======================${NC}"
fi
...
EOF
    echo -e "${GREEN}========== Install APP GenieACS Pragaaaat... ==============${NC}"
    ...
    echo -e "${GREEN}============================================================================${NC}"
    echo -e "${GREEN}=================== GenieACS wis terinstall sedurunge. =====================${NC}"
fi

#Sukses
echo -e "${GREEN}============================================================================${NC}"
echo -e "${GREEN}========== GenieACS UI akses port 3000. : http://$local_ip:3000 ============${NC}"
echo -e "${GREEN}=================== Informasi: Whatsapp 0819-4721-5703 =====================${NC}"
echo -e "${GREEN}============================================================================${NC}"
